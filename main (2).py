...
def get_args():
    ...
    elif config == 'cifar100_trprompt': #DBP trprompt with cifar100
        from configs.cifar100_trprompt import get_args_parser
        config_parser = subparser.add_parser('cifar100_trprompt', help='Split-CIFAR100 TRPrompt configs')
    elif config == 'five_datasets_trprompt': #DBP trprompt with 5-datasets
        from configs.five_datasets_trprompt import get_args_parser
        config_parser = subparser.add_parser('five_datasets_trprompt', help='five datasets TRPrompt configs')
    elif config == 'imr_trprompt': #DBP trprompt with ImageNetR
        from configs.imr_trprompt import get_args_parser
        config_parser = subparser.add_parser('imr_trprompt', help='Split-ImageNet-R TRPrompt configs')
    elif config == 'cub_trprompt': #DBP trprompt with CUB-200
        from configs.cub_trprompt import get_args_parser
        config_parser = subparser.add_parser('cub_trprompt', help='Split-CUB TRPrompt configs')
    else:
...
def main(args):
    ...
    if hasattr(args, 'train_inference_task_only') and args.train_inference_task_only:
        ...
        import trainers.hideprompt_trainer_replay as hideprompt_trainer_replay #DBP: Need to add HiDePrompt with replay
        hideprompt_trainer_replay.train(args)
    ...
    #DBP new implementations
    elif 'trprompt' in args.config and hasattr(args, 'train_inference_task_only') and args.train_inference_task_only: #DBP TRPrompt tii training
        import trainers.trprompt_tii_trainer as trprompt_tii_trainer
        trprompt_tii_trainer.train(args)
    elif 'trprompt' in args.config and not args.train_inference_task_only: #DBP TRPrompt wtp and tap training
        import trainers.trprompt_trainer as trprompt_trainer
        trprompt_trainer.train(args)
    else:
        raise NotImplementedError
