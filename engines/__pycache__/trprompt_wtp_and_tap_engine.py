    ...
    for t, loader in enumerate(data_loader):
        ...
        if t == task_id:
            ...
            for input, target in metric_logger.log_every(loader, args.print_freq, header):
                ...
                compute_ntk_means(task_id, task_labels_perexample[task_id].cpu().tolist(), 0.4, 0.7)
    ...
    for t, loader in enumerate(data_loader):
        ...
        if t < task_id:
            ...
            for input, target in metric_logger.log_every(loader, args.print_freq, header):
                ...
                compute_ntk_means(t, task_labels_perexample[task_id].cpu().tolist(), 0.4, 0.7) 
                ...
                #model_grpo.load_state_dict(clean_state_dict)
                #model_grpo.to(device)
                
                #create a checkpoint of the original optimizer
                optimizer_checkpoint = copy.deepcopy(optimizer.state_dict()) 
                
                new_task_grads_prev_curr = compute_ntk(t, task_id, [input, target, t, prompt_id, set_training_mode, args, class_mask], model, optimizer, checkpoint_model, optimizer_checkpoint, max_norm) #perform transfer if appropriate
                               
                #bring model back to device
                
                task_grads_perexample[t] =  new_task_grads_prev_curr[0]
                task_grads_perexample[task_id] = new_task_grads_prev_curr[1]     
                ...
                optimizer.zero_grad()
                model.module.e_prompt.prompt.grad = task_grads_perexample[t].mean(dim=0) #l2_normalize(task_grads_perexample[t]).mean(dim=0)
                model.module.head.weight.grad = ft_per_sample_grads['head.weight'].mean(dim=0)
                model.module.head.bias.grad = ft_per_sample_grads['head.bias'].mean(dim=0)
                #loss.backward()
                
                #modify 
                #model.module.e_prompt.prompt.grad = modify_grads_perexample(t, task_id).mean(dim=0)
                ...
                #DBP store task grads (before rescaling with max_norm)
                #if model.module.e_prompt.prompt.grad is not None:
                    #task_grads = model.module.e_prompt.prompt.grad.clone() #keys grads of all tasks
                    #model.module.e_prompt.prompt.grad = modify_grads(t, task_id)
                    #print("do nothing")
...
def task_similarity(task_prev: int, task_curr: int, l = 0, key_val = 0, eps = 1e-8):
    ...
    print("Similarity: ", overlap_score)
    
    return overlap_score 
    
    #return torch.dot(task_grads_perexample[task_prev][:][l, key_val, ...].flatten()[torch.nonzero(task_grads_perexample[task_prev][:][l, key_val, ...].flatten())].squeeze(), task_grads_perexample[task_curr][:][l, key_val, ...].flatten()[torch.nonzero(task_grads_perexample[task_curr][:][l, key_val, ...].flatten())].squeeze()) / (task_grads_perexample[task_prev][:][l, key_val, ...].flatten()[torch.nonzero(task_grads_perexample[task_prev][:][l, key_val, ...].flatten())].squeeze().norm() * task_grads_perexample[task_curr][:][l, key_val, ...].flatten()[torch.nonzero(task_grads_perexample[task_curr][:][l, key_val, ...].flatten())].squeeze().norm() + eps)
    
    #implement task correlation by projection
    
def normalize(a, p=2, dim=-1):
    return F.normalize(a, p=2, dim=-1)
    
def compute_ntk(task_prev: int, task_curr: int, input_target: [], model_grpo: torch.nn.Module, optimizer_grpo: torch.optim.Optimizer, checkpoint_model, optimizer_checkpoint, max_norm: float, eps = 1e-8):
    ...
    for l in range(0, 1): #5 layers
        ...
        if task_similarity(task_prev, task_curr, l) > 0.5:

            for i in range(0, len(task_grads_perexample[task_prev])):
        
                for j in range(0, len(task_grads_perexample[task_curr])):
                    
                    #print("TASK SIMILARITY: ", task_similarity(task_prev, task_curr, l, 0).item())
        
                    #if task_grads_perexample[task_prev][i][l, 0, ...].flatten()[torch.nonzero(task_grads_perexample[task_prev][i][l, 0, task_prev, ...].flatten())].shape == task_grads_perexample[task_curr][j][l, 0, task_curr, ...].flatten()[torch.nonzero(task_grads_perexample[task_curr][j][l, 0, task_curr, ...].flatten())].shape and task_grads_perexample[task_prev][i][l, 1, ...].flatten()[torch.nonzero(task_grads_perexample[task_prev][i][l, 1, task_prev, ...].flatten())].shape == task_grads_perexample[task_curr][j][l, 1, task_curr, ...].flatten()[torch.nonzero(task_grads_perexample[task_curr][j][l, 1, task_curr, ...].flatten())].shape:
                        # Compute J(x1) @ J(x2).T
                        #jac1 = task_grads_perexample[task_prev][:, l, 0, task_prev, :, :, :].flatten()[torch.nonzero([task_prev][:, l, 0, task_prev, :, :, :].flatten())]
                        #jac2 = task_grads_perexample[task_curr][:, l, 0, task_curr, :, :, :].flatten()[torch.nonzero([task_curr][:, l, 0, task_curr, :, :, :].flatten())]
                    ...
                    if task_prev_examples_layer_key_normalized[i].shape == task_curr_examples_layer_key_normalized[j].shape:
                        ...
                        if result > 0 and result >= task_ntk_means[task_prev][l][0] and result <= task_ntk_means[task_prev][l][1]:
                            ...
                            #task_grads_perexample[task_prev][i, l, 0, task_prev, ...] = torch.reshape(aligned_keys_layer, (5, 6, 64))
                            #task_grads_perexample[task_prev][i, l, 1, task_prev, ...] = torch.reshape(aligned_values_layer, (5, 6, 64))
                        ...
                        #forward transfer deactivated for now
                        #if result > 0 and result >= task_ntk_means[task_curr][l][0] and result <= task_ntk_means[task_curr][l][1]:
                            #aligned_values_layer = gradient_midpoint(task_prev_examples_layer_value_normalized[i], task_curr_examples_layer_value_normalized[j])

                            #aligned_values_layer = torch.reshape(aligned_values_layer, (5, 6, 64)).unsqueeze(0) / task_curr_examples_layer_value_norm[j]

                            #task_grads_perexample[task_curr][j, l, 1, task_curr, ...] = aligned_values_layer

                            #print("The NTK between ", str(i), " and ", str(j), " at layer ", str(l), " is: ", result)
                            #print("Forward transfer occurred")

        #First evaluate updated using GRPO
    ...
    return [task_grads_perexample[task_prev], task_grads_perexample[task_curr]]
    ...
    for actual_example, aligned_examples in new_aligned_examples_dict.items():
        ...
        if len(old_aligned_examples_dict[actual_example]) > 0: #there is at least one other example to align to
            ...
            for i in range(0, len(aligned_examples)):
                ...
                ratio_aligned_example = torch.mean(log_probs_aligned / log_probs_actual)
            ...
            #print("REWARDS: ", rewards_aligned_examples)
            #print("RATIO: ", ratio_aligned_examples)
            ...
            k = min(3, clipped_rewards_aligned.size(0))
...
def compute_ntk_means(task_curr: int, classes: [], percentile_low = 0.0, percentile_high = 0.0, eps = 1e-8):
    ...
    for l in range(0, num_layers): #5 layers 
        ...
        for cl in set(classes):
            ...
            task_ntk_means[task_curr].append(cl)
            
            task_ntk_means[task_curr][cl] = dict()
            
            cl_ind = [i for i, x in enumerate(classes) if x == cl]
        
            print("CL_IND: ", cl_ind)
            
            print(type(task_ntk_means))
            print(type(task_ntk_means[task_curr]))
            print(type(task_ntk_means[task_curr][cl]))
         ...
         # Compute J(x1) @ J(x2).T
            ...
            if len(result_all_layer) > 0:
                ...
                if len(task_ntk_means[task_curr][cl]) < num_layers:
                    task_ntk_means[task_curr][cl] = {"perc_low": tau_layer[0], "per_high": tau_layer[1]}
                else:
                    alpha = 0.9
                    tau_layer[0] = (1 - alpha) * task_ntk_means[task_curr][cl][l][0] + alpha * tau_layer[0] #update low percentile of current layer with EMA
                    tau_layer[1] = (1 - alpha) * task_ntk_means[task_curr][cl][l][1] + alpha * tau_layer[1] #update high percentile of current layer with EMA
                    
                    task_ntk_means[task_curr][cl][l] = {"perc_low": tau_layer[0], "per_high": tau_layer[1]}
            
                print("Task: ", task_curr, " Class: ", cl, " Layer: ", l, " Percentile: ", tau_layer)
...
def evaluate_till_now(model: torch.nn.Module, original_model: torch.nn.Module, data_loader,
    ...
    if args.output_dir and utils.is_main_process():
        df_acc_matrix.to_csv(os.path.join(args.output_dir, 'accuracies_attask_' + str(task_id) + '.csv'), index = False)
    #DBP end
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module,
    ...
    for task_id in range(args.num_tasks):
        ...
        for epoch in range(args.epochs):
            ...
            #train_stats = train_one_epoch(model=model, original_model_list=original_model_list, criterion=criterion,
            ...
            if epoch_number % align_each_epochs == 0 and task_id > 0:
                print("-----REFINING AT TASK <:----- ", task_id)
                
                for epoch_refine in range(0, 1): #do more epochs later
        ...
        if args.output_dir and utils.is_main_process():
            readable_ntk_means = {
                tid: [
                {
                    ...
                    for cid, class_metrics in item_dict.items()
            }
            for item_dict in list_v
            ]
            for tid, list_v in task_ntk_means.items()
            }       

            with open("task_means_" + str(task_id) + ".txt", "w") as f:
                # indent=4 makes the nested levels look like a clean tree
                json.dump(readable_ntk_means, f, indent=4)        
