...
def evaluate_till_now(model: torch.nn.Module, original_model: torch.nn.Module, data_loader,
    ...
    if args.output_dir and utils.is_main_process():
        df_acc_matrix.to_csv(os.path.join(args.output_dir, 'hideprompt_accuracies_attask_' + str(task_id) + '.csv'), index = False)
    #DBP end
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module,
    ...
    for task_id in range(args.num_tasks):
        ...
        #DBP refine previous tasks by just replaying
        if task_id > 0:
            
            for t in range(0, task_id):
                print("-----REFINING AT TASK <:----- ", task_id)
                
                for epoch_refine in range(0, 1): #online CL for now
                    train_stats = train_one_epoch(model=model, original_model_list=original_model_list, criterion=criterion,
                                                ...
                                                device=device, epoch=epoch, max_norm=args.clip_grad,
                                                set_training_mode=True, task_id=t, class_mask=class_mask,
                                                target_task_map=target_task_map, args=args, )                                            
    ...
    #DBP add train stats before tap but after wtp
        train_stats_before_tap = evaluate_till_now(model=model, original_model=original_model, data_loader=data_loader,
                                       device=device,
    ...
    #DBP add test stats before tap but after wtp                                 
        test_stats_before_tap = evaluate_till_now(model=model, original_model=original_model, data_loader=data_loader,
                                       device=device,
                                       task_id=task_id, class_mask=class_mask, target_task_map=target_task_map,
