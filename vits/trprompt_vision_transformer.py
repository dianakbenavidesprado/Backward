...
class VisionTransformer(nn.Module):
    ...
    #DBP: modified to accept multiple tasks (as a list, returns a list instead of a single dict
    def forward(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, fc_only=False, prompt_momentum=0):
        if fc_only:
            ...
            x = self.fc_norm(x)
            res['logits'] = self.head(x)
            return [res] #DBP changed return list
            
        #DBP changed to handle multiple tasks (as a list) and return list of dict instead of just dict
        if not isinstance(task_id, (list, tuple)):
            task_id = [task_id]
            
        results = []
                
        for tid in task_id:
            res_tid = self.forward_features(x, task_id=tid, prompt_id=prompt_id, prompt_weight=prompt_weight, train=train, prompt_momentum=prompt_momentum)
            res_tid = self.forward_head(res_tid)
            results.append(res_tid)
