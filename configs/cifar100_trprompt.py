...
def get_args_parser(subparsers):
    subparsers.add_argument('--batch-size', default=64, type=int, help='Batch size per device') #DBP changed default to 64
    ...
    #Backward parameters
    subparsers.add_argument("--pct", type=float, default=1.0, help="Fraction of dataset to use") #DBP added extra argument
    subparsers.add_argument('--replay_percentage', default = 0.25, type=float)
    subparsers.add_argument('--with_backward', default = 1, type = int)
    subparsers.add_argument('--with_grpo', default = 1, type = int)
    subparsers.add_argument('--with_forward', default = 1, type = int)
    subparsers.add_argument('--rep', default = 1, type = int)
    subparsers.add_argument('--task_similarity', default = 0.5, type = float)
    subparsers.add_argument('--kappa_low', default = 0.4, type = float)
    subparsers.add_argument('--kappa_high', default = 0.7, type = float)
    subparsers.add_argument('--lambda_update', default = 0.8, type = float)
    subparsers.add_argument('--init_prompt', default = 1, type = int) #whether to initialize prompt the way HiDePrompt does
