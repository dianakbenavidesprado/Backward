...
class SampledDataset(Dataset):
    ...
    def __getitem__(self, idx):
        #DBP modified to return example indices
        #return self.dataset[self.indices[idx]]
        data, target = self.dataset[self.indices[idx]]
        global_idx = self.indices[idx]
        
        return data, target, global_idx
...
def build_continual_dataloader(args):
    ...
    for i in range(args.num_tasks):
        ...
        if args.dataset.startswith('Split-'):
            ...
            #DBP original line (seems to use val twice, instead of train): dataset_train_mean, dataset_val_mean = get_dataset(dataset_list[i], transform_val, transform_val, args)
...
def get_dataset(dataset, transform_train, transform_val, args, target_transform=None):
    if dataset == 'CIFAR100':
        ...
        #DBP Replace targets to fit correct range (task #3 in 5-datasets)
        dataset_train.targets = [t + 20 for t in dataset_train.targets]
        print(np.unique(dataset_train.targets))
        ...
        #DBP Replace targets to fit correct range (task #2 in 5-datasets)
        dataset_train.targets = [t + 10 for t in dataset_train.targets]
        print(np.unique(dataset_train.targets))
        ...
        #DBP Replace targets to fit correct range (task #5 in 5-datasets)
        dataset_train.targets = [t + 40 for t in dataset_train.targets]
        print(np.unique(dataset_train.targets))        
        ...
        #DBP Replace targets to fit correct range (task #4 in 5-datasets)
        dataset_train.targets = [t + 30 for t in dataset_train.targets]
        print(np.unique(dataset_train.targets))
    ...
    #DBP subset train before return - just randomly
    #pct = float(args.pct)
    
    #train_indices = np.random.choice(len(dataset_train), size=int(len(dataset_train) * pct), replace=False)
    #dataset_train_small = SampledDataset(dataset_train, train_indices)
    #print("Original train size: ", len(dataset_train))
    #print("Sampled train size: ", len(dataset_train_small))
    #return dataset_train_small, dataset_val
    
    #DBP stratified random sampling
    targets = np.array(dataset_train.targets)
    train_indices, _ = train_test_split(np.arange(len(targets)), train_size=float(args.pct), stratify=targets, random_state=args.seed)
    dataset_train_small = SampledDataset(dataset_train, train_indices)
    print("Original train size: ", len(dataset_train))
    print("Sampled train size: ", len(dataset_train_small))
    print("Targets: ", np.unique(dataset_train_small.targets))
    return dataset_train_small, dataset_val  
